...
/**
 * Look all files with the specified name in directory and its parents, cache result
 * @param  {string} directory The path of directory to start
 * @returns {array}           A list of matching files
 */
FileFinder.prototype.findAll = function(directory) {
    var result = [];
    /* eslint no-constant-condition:0 */
    while (true) {
        var found = this.findInDirectory(directory);
        if (found === false) {
            return result;
        }

        result.push(found);
        var parentDirectory = path.resolve(path.dirname(found), "..");
