...
var cli = {
     ...
     * @param {Function} callback Function that'll be called after we finished.
     * @returns {void}
     */
    execute: function(args, callback) {
        ...
        if (currentOptions.version) { // version from package.json
            ...
            engine.executeOnFiles(files, function(err, results) {
                if (err) {
                    return callback(err);
                }

                if (printResults(results, currentOptions.format, currentOptions.outputFile)) {
                    callback(null, calculateExitCode(results));
                } else {
                    callback(null, 1);
                }
            });
